  ...
  return (
    <div className="relative flex flex-col w-auto h-fit mb-[60px] border-zinc-800 bg-zinc-950 px-8 py-6 gap-4 z-10">
      <X
        onClick={onClose}
        size={20}
        className="absolute text-zinc-400 right-2 top-2 cursor-pointer hover:text-white z-10"
      />
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex gap-1">
          <button
            onClick={onSubmit}
            className="flex px-4 h-9 items-center bg-zinc-800 hover:bg-white hover:text-zinc-900 rounded text-sm text-white"
          >
            {submitLabel}
          </button>
        </div>
